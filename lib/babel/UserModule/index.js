...
function UserModule() {
    ...
    $registerForm.submit((event) => {
        ...
        for (var i = 0; i < event.target.attributes.length; i++) {
            if (event.target.attributes[i].name === fragArgs.attribute) {
                form.dataAttribute = '[' + event.target.attributes[i].name +
                    '=' + '"' + event.target.attributes[i].value + '"' + ']';
    ...
    function openLoginRegisterForm(args) {
        ...
        var $btnContainerEntity = $('[data-btn-container=' +
            args.entitySelected + ']');
        var $btnContainerNoEntity = $('[data-btn-container=' +
            args.entityNoSelected + ']');
