  ...
  if($('[data-groupmodule-section="group"]').length) {
    userApi.getUserData().then((response) => {
      ...
      signatureApi = new SignatureApi(
        ...
        currentPeriod.fields.name_url);
      return Promise.all(
        [Promise.resolve(currentPeriod), signatureApi.getCurrentSignature()]);
    }).then((data) => {
      let currentPeriod = data[0];
      let currentSignature = data[1];
      ...
      $structureItem.each((index, element) => {
        for (var j = 0; j < element.children.length; j++) {
          element.children[j].setAttribute(
            'href',
            `${baseUrl}/${userApi.username}/${periods[i].fields.name_url}`);
          element.children[j].setAttribute(
            dataAtt,
            periods[i].fields.name_url);
